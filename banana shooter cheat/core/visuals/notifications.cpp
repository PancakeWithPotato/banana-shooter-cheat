...
void CNotifications::AddNotif(std::string text) 
{
	if (this->notifs.size() >= iMaxNotifs) 
	{	this->ClearNotifs();
		Notif_t notif{ .strName = "[meowware] " + text, .dPostTime = ImGui::GetTime()};
